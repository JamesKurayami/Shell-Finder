       ...
       \033[34m
          ...
          [Warning] I am not responsible for the way you will use this program [Warning]
                ...
                | (___ | |__   ___| | | | |__   _ _ __   __| | ___ _ __ 
                 \___ \| '_ \ / _ \ | | |  __| | | '_ \ / _` |/ _ \ '__| 
                 ____) | | | |  __/ | | | |    | | | | | (_| |  __/ |   
                |_____/|_| |_|\___|_|_| |_|    |_|_| |_|\__,_|\___|_|   \033[0m
...
def send_telegram_message(token, chat_id, message):
    ...
    try:
        ...
        if response.status_code == 200:
            print("\033[1;32m\n[+] Message sent to Telegram successfully.\033[0m")
        else:
            print(f"\033[1;31m\n[!] Failed to send message. Error: {response.text}\033[0m")
...
def get_telegram_info():
    ...
    if send_telegram == "yes":
        ...
        try:
            ...
            if response.status_code == 200:
                print("\033[1;32m\n[+] Token is valid.\033[0m")
                return token, chat_id
            else:
                print("\033[1;31m\n[!] Invalid token or chat ID. Please try again.\033[0m")
                return None, None
        except requests.exceptions.RequestException as e:
...
def get_shell_list():
    ...
    print(bcolors.YELLOW + "2. Provide a file containing shells (one per line).")
    
    choice = input("\033[1;32m\n[+] Your choice \033[1;34m(1 or 2): ")
    
    shell_list = []

    if choice == "1":
        print("\033[1;32m\n[+] Using default shell list.\033[0m")
        ...
        try:
            ...
            print(f"\033[1;32m[+] Loaded {len(shell_list)} shells from {filename}.\033[0m")
        except FileNotFoundError:
            print(f"\033[1;31m[!] File '{filename}' not found.\033[0m")
            return []
    else:
...
def get_website_list():
    ...
    print(bcolors.YELLOW + "2. Provide a file containing URLs (one per line).")
    ...
    if choice == "1":
        ...
        while True:
            ...
            if validate_url(website):
                ...
                print("\033\n[1;31m[!] Invalid URL format. Please try again.\033[0m")
    elif choice == "2":
        filename = input(bcolors.GREEN + "\n[+] Enter the filename with the list of URLs: ")
        try:
            with open(filename, 'r') as file:
                website_list = [line.strip() for line in file.readlines() if line.strip()]
            print(f"\033\n[1;32m\n[+] Loaded {len(website_list)} URLs from {filename}.\033[0m")
        except FileNotFoundError:
            print(f"\033\n[1;31m\n[!] File '{filename}' not found.\033[0m")
            return []
    else:
...
def find_shells():
    ...
    if not websites:
        print("\033[1;31m\n[!] No websites provided. Exiting...\033[0m")
        return

    shells = get_shell_list()
    
    if not shells:
        print("\033[1;31m\n[!] No shells provided. Exiting...\033[0m")
        return
            ...
            for shell in shells:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        print(bcolors.RED + f'\n[-] Not Found: {host} {bcolors.YELLOW} '+' [+] Code: 404')
                except requests.exceptions.Timeout:
                    print(bcolors.RED + f'\n[!] Timeout error with {host}')
                except requests.exceptions.RequestException as e:
